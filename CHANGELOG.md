...
### Changed — 压力面板直观化重构（第三批完善）
- **语义化状态词**：状态栏「上下文 N%」改为状态词 + 百分比（充足/正常/接近上限/上下文已满，双语）
- **剩余可输入估算**：面板顶部大字显示「约还能输入 X 万字」（按活动窗口余量反算，中文权重与后端一致）
- **泄压事件一句话时间线**：数字表格改为「最近泄压：剪枝 X 条工具输出 · 移除 Y 条旧消息 · 节省 Z 字符」
- **自动弹出**：发生 HistoryTruncated/PressureEscalation 时面板自动展开（不再需要用户主动发现）；正常泄压仅记录
- 趋势柱高直接按压力比例（0-100%）渲染
