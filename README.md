...
### 🚀 v0.5.0 新增 — 七核心引擎 v2 升级
- **任务智能引擎**：PERT 三点工作量估算 + 多维风险识别 + 关键路径分析
- **多模型协作引擎**：UCB1 探索-利用模型选择（多臂老虎机）
- **预测分析引擎**：CUSUM 累计和变化点检测
- **端侧进化总线**：系统化健康自评估（A-F 分级，已在 EvolutionConsole 展示）
- **多级语义蒸馏引擎**：实体关系图提取（共现 → 关系推断）
- **Web 智能搜索**：相关性重排序（查询词命中 + 来源权威加权）
- **调度引擎**：上下文感知路由（对话历史领域信号）
- 上述 7 项能力均已补齐 IPC 层（`engine_v2` 统一出口）与前端封装

### v0.4.0（上一版）
- 运行模式选择 / 推理深度切换 / 引擎能力去桩 / 前端版本号动态化
